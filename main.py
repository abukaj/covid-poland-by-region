...
def draw_phase_space(df: pandas.DataFrame):
    # These are the colors that will be used in the plot
    color_sequence = ['#1f77b4', '#aec7e8', '#ff7f0e', '#ffbb78', '#2ca02c',
                      '#98df8a', '#d62728', '#ff9896', '#9467bd', '#c5b0d5',
                      '#8c564b', '#c49c94', '#e377c2', '#f7b6d2', '#7f7f7f',
    ...
    ax.xaxis.set_major_formatter(plt.FuncFormatter('{:.3g}'.format))
    ax.yaxis.set_major_formatter(plt.FuncFormatter('{:.3g}'.format))
    ...
    # plt.xticks(x_range, df.date.tolist(), rotation=90, fontsize=10)
    # plt.yticks(np.arange(start=0, step=50, stop=500), fontsize=10)
    for column, color in zip(df.columns[1:], color_sequence):
        new_cases = df[column].tolist()
        line = plt.plot(np.cumsum(new_cases), new_cases, lw=2.5,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--phase-space', dest='draw', action='store_const',
                        const=draw_phase_space, default=draw,
                        help='plot data in phase-space')
    args = parser.parse_args()

    covid = CovidStats()
    df = covid.get_data()
    args.draw(args.normalize(df))
